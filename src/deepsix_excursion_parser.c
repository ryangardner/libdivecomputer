...
typedef struct deepsix_excursion_sample_t {
	unsigned int time;
	unsigned int depth;
	unsigned int temperature;
	unsigned int deco;
	unsigned int ceiling_depth;
	unsigned int ceiling_time;
	unsigned int cns;

	unsigned int have_depth:1;
	unsigned int have_alarm:1;
	unsigned int have_temperature:1;
	unsigned int have_deco:1;
	unsigned int have_ceiling:1;
	unsigned int have_cns:1;
} deepsix_excursion_sample_t;
...
static void deepsix_excursion_report (unsigned int time, deepsix_excursion_sample_t *state, unsigned int atmospheric, dc_sample_callback_t callback, void *userdata)
{
	...
	if (state->have_depth) {
		sample.depth = (signed int)(state->depth - atmospheric) * (BAR / 1000.0) / (DENSITY * GRAVITY);
		if (callback) callback(DC_SAMPLE_DEPTH, sample, userdata);
		state->have_depth = 0;
	}

	if (state->have_temperature) {
		sample.temperature = state->temperature / 10.0;
		if (callback) callback(DC_SAMPLE_TEMPERATURE, sample, userdata);
		state->have_temperature = 0;
	}

	if (state->have_cns) {
		sample.cns = state->cns;
		if (callback) callback(DC_SAMPLE_CNS, sample, userdata);
		state->have_cns = 0;
	}

	if (state->have_deco) {
		if (state->deco >= DecoCal && state->deco <= DecoStop) {
			sample.deco.type = DC_DECO_DECOSTOP;
			if (state->have_ceiling) {
				sample.deco.depth = (signed int)(state->ceiling_depth - atmospheric) * (BAR / 1000.0) / (DENSITY * GRAVITY);
				sample.deco.time = state->ceiling_time;
			} else {
...
deepsix_excursion_parser_samples_foreach (dc_parser_t *abstract, dc_sample_callback_t callback, void *userdata)
{
	...
	deepsix_excursion_sample_t state = {0};
	...
	while (offset + 1 < size) {
		...
		if (type == TEMPERATURE && time > 0) {
			deepsix_excursion_report (time, &state, atmospheric, callback, userdata);
		}
		...
		if (type == ALARM) {
			...
			state.ceiling_depth = ceiling_depth;
			state.ceiling_time = ceiling_time;
			state.have_ceiling = 1;
		} else if (type == CNS) {
